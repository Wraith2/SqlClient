            ...
            { // We only check _inBuff, since two buffers should be consistent.
                ...
                else if (size != _inBuff.Length)
                {
                    ...
                    if (_inBytesRead > _inBytesUsed)
                    {
                        ...
                        SetBuffer(inBuff, 0, remainingData, skipCheck: true);
...
//        }
        ...
        internal void DumpPacket(ReadOnlySpan<byte> bytes)
        {
            ...
            if (bytes.Length >= TdsEnums.HEADER_LEN && bytes[0] == 4 && bytes[1] <= 1 && bytes[7] == 0)
            {
                isFull = "full-";
            }

            string path = _packetRoot + $"packet-{uid}-{bytes.Length}{isFull}.bin";
