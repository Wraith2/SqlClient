...
namespace Microsoft.Data.SqlClient
{
    ...
    internal sealed partial class TdsParser
    {
        ...
        private void FireInfoMessageEvent(SqlConnection connection, SqlCommand command, TdsParserStateObject stateObj, SqlError error)
        {
            ...
            SqlException exc = SqlException.CreateException(sqlErs, serverVersion, _connHandler, command?.GetCurrentBatchCommand());
            ...
            if (temp != null && temp.Count > 0)
            {
                ...
                if (temp.Count == 1 && temp[0].Exception != null)
                {
                    exception = SqlException.CreateException(temp, serverVersion, _connHandler, 
                        batchCommand: command?.GetBatchCommand(temp[0].BatchIndex), 
                        innerException: temp[0].Exception
                    );
                }
                else
                {
                    ...
                    exception = SqlException.CreateException(temp, serverVersion, _connHandler, batchCommand);
        ...
        internal SqlError ProcessSNIError(TdsParserStateObject stateObj)
        {
            ...
            try
            {
                ...
                if (details.sniErrorNumber == 0)
                {
                    ...
                    SqlClientEventSource.Log.AdvancedTraceEvent("<sc.TdsParser.ProcessSNIError |ERR|ADV > ':' character missing in sni errorMessage. Error Message index of ':' = {0}", iColon);
                    Debug.Assert(errorMessage.Length > iColon + 1 && errorMessage[iColon + 1] == ' ', "Expecting a space after the ':' character");
                    SqlClientEventSource.Log.AdvancedTraceEvent("<sc.TdsParser.ProcessSNIError |ERR|ADV > Expecting a space after the ':' character. Error Message Length = {0}", errorMessage.Length);
        ...
        private bool TryProcessFedAuthInfo(TdsParserStateObject stateObj, int tokenLen, out SqlFedAuthInfo sqlFedAuthInfo)
        {
            ...
            if (tokenLen < sizeof(uint))
            {
                ...
                SqlClientEventSource.Log.TraceEvent("<sc.TdsParser.TryProcessFedAuthInfo|ERR> FEDAUTHINFO token stream length too short for CountOfInfoIDs.");
        ...
        internal bool TryProcessError(byte token, TdsParserStateObject stateObj, SqlCommand command, out SqlError error)
        {
            ...
            error = new SqlError(number, state, errorClass, _server, message, procedure, line, null, batchIndex);
            return true;
        }
            ...
            bool isReturnValue)
        {
            ...
            if (cipherTable.HasValue)
            {
                ...
                {
                    SqlClientEventSource.Log.TraceEvent("<sc.TdsParser.TryProcessTceCryptoMetadata|TCE> Incorrect ordinal received {0}, max tab size: {1}", index, cipherTable.Value.Size);
